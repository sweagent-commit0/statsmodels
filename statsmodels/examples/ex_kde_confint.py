...
x = mixture_rvs([0.25, 0.75], size=200, dist=[stats.norm, stats.norm], kwargs=(dict(loc=-1, scale=0.5), dict(loc=1, scale=0.5)))
x.sort()
...
kernel_names = ['Biweight', 'Cosine', 'Epanechnikov', 'Gaussian', 'Triangular', 'Triweight']
fig = plt.figure()
for ii, kn in enumerate(kernel_names):
    ...
    ax.fill_between(x_grid, confint_grid[:, 0], confint_grid[:, 1], color='grey', alpha='0.7')
