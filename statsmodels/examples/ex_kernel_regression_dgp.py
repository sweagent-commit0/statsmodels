...
if __name__ == '__main__':
    ...
    print(seed)
    np.random.seed(seed)
    funcs = [dgp.UnivariateFanGijbels1(), dgp.UnivariateFanGijbels2(), dgp.UnivariateFanGijbels1EU(), dgp.UnivariateFunc1()]
    res = []
    fig = plt.figure()
    for i, func in enumerate(funcs):
        f = func
        model = KernelReg(endog=[f.y], exog=[f.x], reg_type='ll', var_type='c', bw='cv_ls')
        mean, mfx = model.fit()
